...
import { createMisc } from "@/lib/api/work-order";
import { getSalesCodes } from "@/lib/api/customer";
import { getSectionsList } from "@/lib/api/work-order";

export default function AddMiscDialog({ open, onOpenChange, wo, token, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const saleCode = form.SaleCode || defaultSaleCode;
    if (!saleCode) {
    ...
    try {
      ...
      await createMisc(
        {
          ...
          Sell: form.Sell ? parseFloat(form.Sell) : (form.Cost ? parseFloat(form.Cost) : 0),
      ...
      setError(err?.message || "Failed to create misc charge");
      toast.error(err?.message || "Failed to create misc charge");
    } finally {
      setSaving(false);
    }
  };
