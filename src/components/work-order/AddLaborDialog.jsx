...
import { createLabor } from "@/lib/api/labor";
...
export default function AddLaborDialog({ open, onOpenChange, wo, token, onSuccess }) {
  ...
  useEffect(() => {
    ...
    setForm({
      DateOfLabor: new Date().toISOString().slice(0, 10),
      EmployeeNumber: "",
      MechanicName: "",
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.EmployeeNumber || !form.MechanicName) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent size="default" className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Add New Labor Entry</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Select
                value={form.SaleCode || (typeof wo?.SaleCode === "object" ? wo?.SaleCode?.Code : wo?.SaleCode) || ""}
