...
export default function ReportsPage() {
  ...
  const fetchData = useCallback(() => {
    ...
    setLoading(true);
    Promise.all([
      getPendingInvoicesSearch(debouncedDates.start, debouncedDates.end),
      getOpenClosedSummary(debouncedDates.start, debouncedDates.end),
      getLaborClosedReport(debouncedDates.start, debouncedDates.end),
      getLaborTotalReport(debouncedDates.start, debouncedDates.end),
    ])
      .then(([inv, oc, cr, lr]) => {
        setInvoices(inv);
        setOpenClosed(oc);
