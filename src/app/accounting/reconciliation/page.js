...
export default function ReconciliationPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const s = toYMD(filters.startDate);
      const e = toYMD(filters.endDate);
  ...
  const matchSelected = useCallback(async () => {
    ...
    try {
      const payload = {
        ...
        bankIds,
        period: { startDate: toYMD(filters.startDate), endDate: toYMD(filters.endDate) },
      };
    ...
    try {
      for (const p of pairs) {
        await postReconciliationMatches(
          { accountingIds: [p.acctId], bankIds: [p.bankId], period: { startDate: toYMD(filters.startDate), endDate: toYMD(filters.endDate) } },
          token
        );
      }
      await loadData();
      ...
      <div className="mx-auto max-w-[1600px] px-4 py-6 sm:px-6 lg:px-8">
        ...
        >
          <div className="grid grid-cols-1 gap-4 md:grid-cols-6">
            <div>
              <label className="mb-1 block font-medium text-slate-700 dark:text-slate-300">Start Date</label>
              <input
                type="date"
                value={toYMD(filters.startDate)}
                onChange={(e) => setFilters((f) => ({ ...f, startDate: e.target.value ? new Date(e.target.value) : f.startDate }))}
                className="w-full rounded-lg border border-slate-300 bg-white p-2.5 dark:border-slate-600 dark:bg-slate-800"
              />
            </div>
            <div>
              <label className="mb-1 block font-medium text-slate-700 dark:text-slate-300">End Date</label>
              <input
                type="date"
                value={toYMD(filters.endDate)}
                onChange={(e) => setFilters((f) => ({ ...f, endDate: e.target.value ? new Date(e.target.value) : f.endDate }))}
                className="w-full rounded-lg border border-slate-300 bg-white p-2.5 dark:border-slate-600 dark:bg-slate-800"
              />
            </div>
            <div>
