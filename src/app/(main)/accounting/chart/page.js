...
export default function ChartOfAccountsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const data = await getAccountsWithTotals(
        dates.startDate,
        dates.endDate,
        token
      );
  ...
  }, [token, dates.startDate, dates.endDate]);
  ...
  const accounts = useMemo(
    () => (rawData ? processAccounts(rawData, dates.endDate) : []),
    [rawData, dates.endDate]
  );
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2 sm:items-end">
            <div className="flex items-center gap-2">
              <input
                type="date"
                value={toYMD(dates.startDate)}
                onChange={(e) =>
                  setDates((d) => ({
                    ...d,
                    startDate: e.target.value ? new Date(e.target.value) : d.startDate,
                  }))
                }
              ...
              <input
                type="date"
                value={toYMD(dates.endDate)}
                onChange={(e) =>
                  setDates((d) => ({
                    ...d,
                    endDate: e.target.value ? new Date(e.target.value) : d.endDate,
                  }))
                }
                className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-800"
              />
