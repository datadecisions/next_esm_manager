...
export default function LaborKPIPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [laborByTech, openSum, overviewData, timecards] = await Promise.all([
        branchNum
          ? getLaborByTechByBranch(branchNum, token, { startDate, endDate })
          : Promise.resolve({ techs: [] }),
        branchesParam
          ? getOpenLaborSummary({ branches: branchesParam })
          : Promise.resolve(null),
        getLaborOverview({ startDate, endDate }),
        branchesParam
          ? getLaborTimecardsReport({
  ...
  }, [token, branchNum, branchesParam, branches.length, startDate, endDate]);
